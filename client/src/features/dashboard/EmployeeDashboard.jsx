  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <QuickAction to="/my-leave?action=new" icon={Plane} label="Apply leave" description="Request time off" tone="bg-sky-50 text-sky-600" />
        <QuickAction
          to="/my-status?action=new"
          icon={ClipboardPen}
        ...
        />
        <QuickAction to="/sales?action=new" icon={BriefcaseBusiness} label="Add sale" description="Log a new lead" tone="bg-violet-50 text-violet-600" />
        <QuickAction to="/calendar" icon={CalendarDays} label="View calendar" description="Holidays & events" tone="bg-amber-50 text-amber-600" />
      </div>

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Leave taken this year" value={`${leave.approvedDaysThisYear} ${leave.approvedDaysThisYear === 1 ? 'day' : 'days'}`} icon={CalendarCheck2} tone="brand" to="/my-leave" />
        <StatCard label="Pending leave requests" value={leave.pendingCount} icon={Hourglass} tone="amber" to="/my-leave?status=pending" />
        <StatCard label="Reports this month" value={dailyStatus.thisMonthCount} icon={ClipboardList} tone="green" to="/my-status" />
        <StatCard
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <Card className="xl:col-span-2">
          <CardHeader icon={ClipboardList} title="Today’s status report" action={<ViewAll to="/my-status" label="History" />} />
          <div className="px-5 pb-5">
            {dailyStatus.today ? (
              ...
              <div className="flex flex-col items-center rounded-xl border-2 border-dashed border-slate-200 px-6 py-8 text-center">
                ...
                <Button as={Link} to="/my-status?action=new" className="mt-4" leftIcon={Plus}>
                  Add today’s status
                </Button>
              </div>
            )}
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <Card>
          <CardHeader icon={Plane} title="My recent leave" action={<ViewAll to="/my-leave" />} />
          {leave.next && (
          ...
          {leave.recent.length === 0 ? (
            ...
            <ul className="divide-y divide-slate-100 border-t border-slate-100">
              {leave.recent.map((item) => (
                <li key={item.id}>
                  <Link to={`/my-leave?leave=${item.id}`} className="flex items-center justify-between gap-3 px-5 py-3 transition-colors hover:bg-slate-50">
        ...
        <Card>
          <CardHeader icon={ClipboardList} title="Recent reports" action={<ViewAll to="/my-status" />} />
